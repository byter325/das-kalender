...
export module XMLManager {
    ...
    export function getGroupByUid(uid: string): any | null {
        try {
            ...
            var path = `${dataGroups}/${Utils.GenSHA256Hash(uid)}.xml`
    ...
    export function getEventByHashAndEventUID(uid: string, eventUid: string, subPath: string): any {
        try {
            const parser = new XMLParser()
            var data = fs.readFileSync(`${dataDir}/${subPath}/${Utils.GenSHA256Hash(uid)}.xml`)
    ...
    export function insertUser(
        ...
        try {
            ...
            writeFileSync(`${dataUsers}/${Utils.GenSHA256Hash(user.uid)}.xml`, xmlDataStr, { flag: "w+" })
            writeFileSync(`${dataUserEvents}/${Utils.GenSHA256Hash(user.uid)}.xml`, "<events></events>", { flag: "w+" })
            return true
        } catch (e) {
            console.log(e);
            return false
    ...
    export function insertGroup(uid: string, name: string): boolean {
        try {
            ...
            writeFileSync(`${dataGroups}/${Utils.GenSHA256Hash(uid)}.xml`, xmlDataStr, { flag: "w+" })
            writeFileSync(`${dataGroupEvents}/${Utils.GenSHA256Hash(uid)}.xml`, "<events></events>", { flag: "w+" })
            return true
        } catch (e) {
            console.log(e);
            return false
    ...
    function insertEvent(uid: string, subPath: string, event: CalendarEvent): boolean {
        try {
            ...
            writeFileSync(`${dataDir}/${subPath}/${Utils.GenSHA256Hash(uid)}.xml`, xmlDataStr, { flag: "w+" })
            return true
        } catch (e) {
            console.log(e);
            return false
    ...
    export function getAllEvents(uid: string, subPath: string): any {
        try {
            const parser = new XMLParser()
            var data = fs.readFileSync(`${dataDir}/${subPath}/${Utils.GenSHA256Hash(uid)}.xml`, { encoding: "utf-8" })
            var events = parser.parse(data)["events"]
            return events
        } catch (error) {
            console.log(error);
    ...
    export function deleteUser(uid: string): boolean {
        try {
            fs.rmSync(`${dataUsers}/${Utils.GenSHA256Hash(uid)}.xml`)
            fs.rmSync(`${dataUserEvents}/${Utils.GenSHA256Hash(uid)}.xml`)
            return true
        } catch (e) {
            console.log(e)
            return false
    ...
    export function deleteGroup(uid: string): boolean {
        try {
            fs.rmSync(`${dataGroups}/${Utils.GenSHA256Hash(uid)}.xml`)
            fs.rmSync(`${dataGroupEvents}/${Utils.GenSHA256Hash(uid)}.xml`)
            return true
        } catch (e) {
            console.log(e)
            return false
